...
export default function History() {
  ...
  async function buildReceiptElement(tx) {
    ...
    // map fields
    const senderName = 'ElitePay Wallet';
    ...
    const remark = tx.meta?.remark || (status === 'successful' || status === 'claimed' ? 'Transaction Successful' : 'Processed by ElitePay');
    ...
    wrap.innerHTML = `
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        <div style="display:flex;gap:14px;align-items:center">
          <img src="/elitepay-logo.png" alt="logo" style="width:72px;height:72px;object-fit:contain"/>
          <div>
            <div style="font-weight:800;font-size:20px;color:#071224">ElitePay Wallet</div>
      ...
      <div style="margin-top:14px;font-size:11px;color:#6b7280">
        This receipt is issued by ElitePay Wallet. For support contact the app.
      </div>
    `;
  ...
  async function downloadReceiptPdf(tx) {
    try {
      ...
      await downloadDataUri(pdfDataUri, `elitepay_receipt_${id}.pdf`);
    } catch (e) {
      console.error(e);
  ...
  async function downloadReceiptJpg(tx) {
    try {
      ...
      const id = (tx.id || tx.created_at || Date.now()).toString().replace(/[^0-9]/g,'');
      await downloadDataUri(jpgDataUri, `elitepay_receipt_${id}.jpg`);
    } catch (e) {
      console.error(e);
          ...
          <div className="introBox card" onClick={(e)=>e.stopPropagation()} style={{maxWidth:760}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center', marginBottom:10}}>
              <div style={{display:'flex',gap:12,alignItems:'center'}}>
                <img src="/elitepay-logo.png" alt="logo" style={{width:48,height:48}} />
                <div>
                  <div style={{fontWeight:800}}>ElitePay Wallet</div>
            ...
            <div ref={receiptRef} style={{padding:16, background:'#fff', color:'#071224', borderRadius:8}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center', marginBottom:8}}>
                <div>
                  <img src="/elitepay-logo.png" alt="logo" style={{width:72,height:72}} />
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontWeight:800, fontSize:18}}>Transaction Receipt</div>
                  <div className="small muted" style={{fontSize:12}}>Generated by ElitePay Wallet</div>
                </div>
              </div>
              ...
              <table style={{width:'100%', borderCollapse:'collapse', color:'#071224'}}>
                <tbody>
                  ...
                  <tr><td style={{padding:'8px 6px', color:'#6b7280'}}>Sender</td><td style={{padding:8,fontWeight:700}}>ElitePay Wallet</td></tr>
              ...
              <div style={{marginTop:12, fontSize:12, color:'#555'}}>This receipt was generated by ElitePay Wallet. If you have questions contact support via the app.</div>
            </div>
          </div>
        </div>
      )}
