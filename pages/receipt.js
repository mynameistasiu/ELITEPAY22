...
export default function ReceiptPage() {
  ...
  async function downloadSvg() {
    ...
    a.download = `elitepay_receipt_${id}.svg`;
  ...
  async function downloadJpg() {
    ...
    setGenerating(true);
    try {
      await svgToImageDownload(svgDataUri, `elitepay_receipt_${(receipt.id||receipt.created_at||Date.now()).toString().replace(/[^0-9]/g,'')}.jpg`, "image/jpeg", 0.92);
    } catch (e) {
      console.error(e);
  ...
  function shareWhatsapp() {
    ...
    const msg = encodeURIComponent(`I just completed a transaction on ElitePay Wallet.\nReference: ${id}\nAmount: ₦${Number(amount).toLocaleString()}\nCheck your ElitePay app.`);
    ...
    <div style={{ maxWidth: 920, margin: "18px auto", padding: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <img src="/elitepay-logo.png" alt="logo" style={{ width: 64, height: 64 }} />
          <div>
            <div style={{ fontWeight: 800, fontSize: 20 }}>ElitePay Wallet</div>
...
async function buildReceiptSvgString(tx) {
  ...
  const watermark = `<g opacity="0.06" transform="translate(${width / 2 - 120}, ${400}) rotate(-15)">
    <text x="0" y="0" font-size="64" font-weight="800" fill="#071224">ElitePay Wallet</text>
  </g>`;

  // sanitize text (basic)
  const esc = s => String(s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

  // inline logo image — we reference /elitepay-logo.png (same origin). Use <image>.
  // Build SVG content
  const svg = `
  <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="880" viewBox="0 0 ${width} 880" style="font-family: Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial;">
    ...
    <g transform="translate(${padding}, ${padding})">
      <image href="/elitepay-logo.png" width="72" height="72" x="0" y="0" />
      <g transform="translate(92,10)">
        <text x="0" y="14" font-size="20" font-weight="800" fill="#071224">ElitePay Wallet</text>
    ...
    <g transform="translate(${padding}, ${padding + 92 + 180})">
      ...
      <g transform="translate(0,18)">
        ...
        <text x="240" y="24" class="value">${esc("ElitePay Wallet")}</text>
    ...
    <g transform="translate(${padding}, ${padding + 92 + 180 + 300})">
      <text x="0" y="18" class="small">This receipt is issued by ElitePay Wallet. Contact support via the app for questions.</text>
      <text x="${width - padding - 220}" y="18" class="muted" font-size="11">Verified • ElitePay Wallet</text>
    </g>
  </svg>
  `;
